...
namespace paper_toolkit::motion_library
{
    namespace
    {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(_directory, ec)) {
            ...
            if (stream) {
                ...
                if (parse(buffer.str(), library, nullptr) && library.curated) {
                    PAPER_TOOLKIT_LOG_INFO(Config, "Motion library wipe: keeping curated file '{}'", entry.path().filename().string());
                    continue;
                }
            }
            stream.close();
    ...
    void MotionLibraryStore::writerLoop()
    {
        ...
        for (;;) {
            ...
            const auto tempPath = write.path + ".tmp";
            {
                ...
                if (!stream) {
                    PAPER_TOOLKIT_LOG_WARN(Config, "Motion library: could not open '{}' for writing", tempPath);
                    continue;
                }
                stream.write(write.content.data(), static_cast<std::streamsize>(write.content.size()));
                if (!stream) {
                    PAPER_TOOLKIT_LOG_WARN(Config, "Motion library: write to '{}' failed", tempPath);
                    continue;
                }
            }
