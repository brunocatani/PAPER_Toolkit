...
namespace paper_toolkit
{
    namespace
    {
        constexpr char kSandboxConsumerName[] = "PAPER_Toolkit";
    ...
    bool WeaponPartDriveSandbox::ensureRegistered()
    {
        ...
        // from the dllimport symbol — PAPER_Toolkit does not link ROCK.lib.
    ...
    void WeaponPartDriveSandbox::ensureTargetsInstalled(const FrameInput& input)
    {
        ...
        if (count == 0) {
            ...
            _installedTargets = {};
            PAPER_TOOLKIT_LOG_INFO(Weapon, "WeaponPartDriveSandbox: attach-only targets cleared (no eligible moving parts)");
            return;
        }
